...
public class WalletQueryController {
    ...
    @GetMapping("/point")
    @Operation(summary = "포인트 결제 내역 조회", description = "선결제 / 금액사용 각 리스트 조회"
    , responses = {
            @ApiResponse(responseCode = "200", description = "OK"
            , content = @Content(schema = @Schema(implementation = BaseResponse.class)))
    })
    @CommonApiResponse
    public ResponseEntity<BaseResponse<?>>getPoint(Authentication authentication
    , @RequestParam(required = true) Boolean prePay
    , @RequestParam(required = false) LocalDate createdAfter
    , @RequestParam(required = false) LocalDate createdBefore
    , @PageableDefault(page = 0, size = 3, sort = "createdAt", direction =  Sort.Direction.DESC) Pageable pageable) {
        PointResponse pointResponse = walletQueryService.getPointList(AuthenticationParser.parseUserId(authentication), prePay, createdAfter, createdBefore, pageable);
